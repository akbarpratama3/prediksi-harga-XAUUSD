...
# Ambil data terbaru untuk prediksi, misalnya 60 hari terakhir
...
# Inisialisasi array untuk menyimpan prediksi 10 hari ke depan
predictions_10_days = []

# Prediksi selama 10 hari ke depan
for _ in range(20):
    # Prediksi hari berikutnya
    prediksi_hari_berikutnya = model.predict(x_input)

    # Inverse transform untuk mendapatkan nilai asli
    prediksi_hari_berikutnya_scaled = sc.inverse_transform(prediksi_hari_berikutnya)

    # Tambahkan prediksi ke dalam list
    predictions_10_days.append(prediksi_hari_berikutnya_scaled[0][0])

    # Update x_input dengan prediksi terbaru untuk prediksi berikutnya
    # Bentuk ulang prediksi untuk memasukkan ke dalam x_input
    x_input = np.append(x_input[:, 1:, :], [[prediksi_hari_berikutnya[0]]], axis=1)

# Buat DataFrame untuk prediksi 10 hari ke depan dengan tanggal
prediction_dates = pd.date_range(df.index[-1] + pd.Timedelta(days=1), periods=20)
predicted_df = pd.DataFrame(predictions_10_days, index=prediction_dates, columns=['Prediksi Harga Penutupan (USD)'])

# Format DataFrame
predicted_df.reset_index(inplace=True)
predicted_df.columns = ['Tanggal', 'Prediksi Harga Penutupan (USD)']

# Tampilkan hasil prediksi selama 10 hari ke depan
print(predicted_df)
